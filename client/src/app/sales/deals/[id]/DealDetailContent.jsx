...
export default function DealDetailContent({ initialDeal, initialMeetings = [], initialSamples = [], initialFollowUps = [] }) {
  ...
  // (WON flow now uses /sales/deals/[id]/convert page)
  ...
  const handleStageChange = (newStage) => {
    ...
    if (newStage === "WON") {
      ...
      router.push(`/sales/deals/${deal.id}/convert`);
      return;
    }
        ...
        actions={
          <div className="flex items-center gap-3">
            {deal.stage !== "WON" && deal.stage !== "LOST" && (
              <Link
                href={`/sales/deals/${deal.id}/proposal`}
        ...
        <div className="bg-gradient-to-r from-emerald-500 to-green-600 rounded-[24px] p-6 lg:p-8 shadow-xl shadow-emerald-500/20 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="flex flex-wrap gap-3">
              {deal.client && (
                <Link
                  href={`/sales/clients/${deal.client.id}`}
              ...
              {deal.project && (
                <Link
                  href={`/sales/projects/${deal.project.id}`}
                  className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-950/20 hover:bg-white/30 text-white text-sm font-medium rounded-xl transition-all"
                >
