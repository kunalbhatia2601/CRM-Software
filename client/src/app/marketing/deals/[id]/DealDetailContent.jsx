...
export default function DealDetailContent({ initialDeal, initialMeetings = [], initialSamples = [], initialFollowUps = [] }) {
  ...
  const handleStageChange = (newStage) => {
    ...
    if (newStage === "WON") {
      // Conversion creates a client and project, which marketing cannot do.
      // Sales or the account manager closes it out.
      showToast("error", "Ask sales or the account manager to convert this deal to a project.");
      return;
    }
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: "Deals", href: "/marketing/deals" },
          { label: deal.title },
        ]}
        actions={
          <div className="flex items-center gap-3">
            {deal.stage !== "WON" && deal.stage !== "LOST" && (
              <Link
                href={`/marketing/deals/${deal.id}/proposal`}
            ...
            {deal.stage !== "WON" && (
              <Link
                href={`/marketing/deals/${deal.id}/edit`}
        ...
        <div className="bg-gradient-to-r from-emerald-500 to-green-600 rounded-[24px] p-6 lg:p-8 shadow-xl shadow-emerald-500/20 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="flex flex-wrap gap-3">
              {deal.client && (
                <Link
                  href={`/marketing/clients/${deal.client.id}`}
              ...
              {deal.project && (
                <Link
                  href={`/marketing/projects/${deal.project.id}`}
                  className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-950/20 hover:bg-white/30 text-white text-sm font-medium rounded-xl transition-all"
                >
                ...
                <div key={ds.id} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 hover:border-slate-200 transition-colors">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <Link href={`/marketing/services/${ds.service?.id}`} className="text-sm font-semibold text-slate-900 dark:text-slate-50 hover:text-indigo-600 transition-colors">
                        {ds.service?.name}
                      </Link>
