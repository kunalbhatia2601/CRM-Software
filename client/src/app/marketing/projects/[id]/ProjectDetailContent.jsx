...
export default function ProjectDetailContent({ initialProject, initialMeetings = [], initialDocuments = [], initialSteps = [], initialTasks = [], initialMilestones = [], assignableUsers = [] }) {
  ...
  // Deep-linked from a task list elsewhere — forces the view to wherever the
  // task actually lives, so the scroll-to in PlanningSection has something to find.
  const highlightTaskId = searchParams.get("task");

  const [planningView, setPlanningView] = useState("list"); // "list" | "kanban"
  // Marketing works two ways on a project: delivery, and campaign performance.
  const [mainTab, setMainTab] = useState("overview") // "overview" | "analytics"
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1 pt-4">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                Client:{" "}
                <Link
                  href={`/marketing/clients/${project.client?.id}`}
        ...
        <div className="bg-white dark:bg-slate-950 rounded-[24px] p-6 lg:p-8 border border-slate-100 dark:border-slate-800 shadow-sm dark:shadow-none shadow-slate-200/50 dark:shadow-none">
          ...
          <div className="flex flex-col">
            <InfoRow
              ...
              link={`/marketing/clients/${project.client.id}`}
            />
            <InfoRow
              icon={User}
              label="Contact"
            ...
            {project.deal && (
              <>
                <InfoRow
                  ...
                  link={`/marketing/deals/${project.deal.id}`}
                />
                <InfoRow
                  icon={DollarSign}
                  label="Deal Value"
      ...
      {/* ═══ Invoices ═══ */}
      <ProjectInvoicesSection projectId={project.id} basePath="/marketing" />

      <ProjectExpensesSection projectId={project.id} basePath="/marketing" />
