 ...
 * someone else and still need to track. The owner additionally gets a "whose
 * tasks" dropdown on the first tab — Mine by default, or All, or any one
 * employee — since an owner is checking on other people's work as often as
 * their own.
 */
const SCOPES = [
  {
...
export default function OwnerTasksContent({ initialTasks = [], staff = [] }) {
  ...
  // Whose tasks to show: "mine" (default), "all", or a specific user id. Only
  // meaningful on the "mine" scope tab — "Assigned by Me" is always about the
  // signed-in owner's own delegations, regardless of who is picked here.
  const [assigneeFilter, setAssigneeFilter] = useState("mine");
  // Deep link from the dashboard: /owner/tasks?task=<id> opens that task.
  ...
  }, [scope, statusFilter, priorityFilter, assigneeFilter]);
  ...
  // The quick-advance button changes a task's status as its assignee — only
  // sound when the tasks on screen are actually the signed-in owner's own.
  const isOwnTasks = !isDelegated && assigneeFilter === "mine";
  const selectedStaff = staff.find((u) => u.id === assigneeFilter);

  const blurb = isDelegated
    ? activeScope.blurb
    : assigneeFilter === "all"
      ? "Every task in the CRM, across every project and assignee."
      : assigneeFilter === "mine"
        ? activeScope.blurb
        : `${selectedStaff?.name || "Their"}\u2019s tasks across all projects.`;
  const emptyMessage = isDelegated
    ? activeScope.empty
    : assigneeFilter === "all"
      ? "No tasks recorded anywhere yet."
      : assigneeFilter === "mine"
        ? activeScope.empty
        : `${selectedStaff?.name || "This person"} has no tasks assigned yet.`;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{blurb}</p>
      </div>
        ...
        {!isDelegated && (
          <div className="relative">
            ...
            <select
              ...
              className="pl-8 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-slate-50 focus:ring-2 focus:ring-[#5542F6] focus:border-transparent outline-none"
            >
      ...
      {loading ? (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                      ...
                      {(isDelegated || (!isOwnTasks && task.assignee)) && task.assignee && (
                        <span className="inline-flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                          <User className="w-3 h-3" />
                          {task.assignee.firstName} {task.assignee.lastName}
                        </span>
                      )}
                      {isOwnTasks && ASSIGNEE_NEXT[task.status] && (
                        <button
        ...
        <div className="bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 p-12 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400">
            {statusFilter === "ALL" ? emptyMessage : `No ${statusFilter.replace("_", " ").toLowerCase()} tasks.`}
          </p>
        </div>
      )}
    </div>
